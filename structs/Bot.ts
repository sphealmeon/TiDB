...
export class Bot {
    ...
    public connection = require('../database/db');
    ...
    public async logData(violationRating:string, userId: string){
      this.client.on('guildCreate', async(guild:Guild)=>{
        try{
          await this.connection.query(
            `INSERT INTO Guilds VALUES('${guild.id}', '${guild.ownerId}', '${violationRating}', '${userId}')`
          );
        }catch(err){
          console.log(err);
        }
